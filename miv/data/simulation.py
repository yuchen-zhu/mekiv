...
parser.add_argument(
    "--data-size", type=int, default=data_size, help="set dataset size to generate."
)
parser.add_argument(
    "--do-save-scm",
    type=bool,
    default=do_save_scm,
    help="set dataset size to generate.",
)
...
def _normal_noise(mean, std, size=-1):
    if size == -1:
        raise ValueError(
            "need to set number of data points as input into noise function."
        )
...
class MerrorIV:
    ...
    def __init__(self, **config):
        ...
        self.Z_dim, self.M_dim, self.N_dim = (
            config["Z_dim"],
            config["M_dim"],
            config["N_dim"],
        )
        self.X_dim, self.U_dim, self.Y_dim = (
            config["X_dim"],
            config["U_dim"],
            config["Y_dim"],
        )
        ...
        self.generate_random_discrete_factors = config[
            "generate_random_discrete_factors"
        ]
    ...
    def _build_graph(self):
        ...
        self.DAG_order = cat(
            [self.Z, self.U, self.X, self.M, self.N, self.Y], axis=0
        )  # an array containing all variables ordered from source code(s) to sink nodes
        num_variables = self.DAG_order.shape[0]
        self.rev_DAG_order = np.zeros(
            (num_variables,), dtype=int
        )  # an array which tells the position of the node in the graph
        ...
        for i in range(num_variables):
            if self.is_Z[i]:
                ...
                DAG_parents.append(
                    np.intersect1d(self.DAG_order[:i], cat([self.Z, self.U]))
                )
            elif self.is_M[i]:
                DAG_parents.append(
                    np.intersect1d(self.DAG_order[:i], cat([self.X, self.M]))
                )
            elif self.is_N[i]:
                DAG_parents.append(
                    np.intersect1d(self.DAG_order[:i], cat([self.X, self.N]))
                )
            elif self.is_Y[i]:
                DAG_parents.append(
                    np.intersect1d(self.DAG_order[:i], cat([self.U, self.X, self.Y]))
                )
        ...
        for i in range(num_singleton_variables):
            if self.is_discrete[i]:
                if self.generate_random_discrete_factors:
                    ...
                    self.scms.append(
                        gen_discrete_factor(
                            pa_categories=pa_categories, ch_categories=ch_categories
                        )
                    )
                else:
                ...
                raise NotImplementedError
                noisy_scm = construct_noisy_scm(
                    scm=_sum_function,
                    scm_params_dict={},
                    noise_fn=_normal_noise,
                    noise_params_dict=noise_config,
                )
    ...
    def generate(self, data_size):
        ...
        for i in range(num_singleton_variables):
            ...
            if self.is_discrete[i]:
                ...
                sample_fn = lambda prob: np.random.choice(
                    np.arange(ch_categories), p=prob
                )
                prob_matrix = (
                    scm_i[tuple(pa_data.T)]
                    if len(pa) > 0
                    else np.tile(scm_i, [data_size, 1])
                )
                assert np.abs(np.sum(prob_matrix[0]) - 1.0) < 1e-7
                ch_data = np.apply_along_axis(
                    func1d=sample_fn, axis=-1, arr=prob_matrix
                )
            else:
                scm_i = self.scms[i]
...
def gen_plots(data_df, model, sem_name):
    ...
    sns.pairplot(data_df, kind="hist"), plt.savefig(
        path_cat(save_dir, "full_pairwise.png")
    ), plt.close()
