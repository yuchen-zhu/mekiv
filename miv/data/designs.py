...
class Datasets:
    ...
    def construct_custom1(self):
        ...
        custom1_design.fu = lambda N_data: np.random.normal(
            0, 1.5, size=N_data
        ).reshape(-1, 1)
        custom1_design.fz = lambda N_data: np.random.normal(
            0, 1.5, size=N_data
        ).reshape(-1, 1)
        custom1_design.fx = (
            lambda z, u, N_data: u
        ...
        custom1_design.fy = (
            lambda x, u, N_data: fns.sigmoid(x)
            + 0.05 * u
            + custom1_design.noise_level * np.random.normal(0, 1, N_data).reshape(-1, 1)
        )
        custom1_design.fm = (
    ...
    def construct_NP(self):  # Newey-Powell 'sigmoid' design
        ...
        NP_design.futw = lambda N_data: np.random.multivariate_normal(
            NP_design.MU, NP_design.SIGMA, size=N_data
        )
        ...
        NP_design.fx = lambda z, u, N_data: stats.norm.cdf(
            (NP_design.utw[:, 1] + NP_design.utw[:, 2]) / np.sqrt(2)
        ).reshape(-1, 1)
        NP_design.fy = (
            lambda x, u, N_data: np.log(np.abs(16 * x - 8) + 1) * np.sign(x - 0.5) + u
        )
        NP_design.fm = (
            lambda x, N_data: x
    ...
    def construct_HLLT(self):  # demand design
        ...
        HLLT_.ft = (
            lambda N_data: np.random.uniform(0, 1, HLLT_.N_data).reshape(-1, 1) * 10
        )

        HLLT_.fu = lambda N_data: np.random.normal(0, 1, HLLT_.N_data).reshape(
            -1, 1
        )  # noise price
        ...
        HLLT_.ydox = (
            lambda x: 100 + (10 + x[:, 0]) * x[:, 2] * get_psi(x[:, 1]) - 2 * x[:, 0]
        )
