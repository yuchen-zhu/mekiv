...
class Datasets:
        ...
        custom1_design = DotDict(
            {
                ...
                "N_data": 10000,
                "noise_level": 0.0,
                "noise_level_merror": 3.0,
                "merror_dim": 0,
                "bias": 0.0,
            }
        )
        custom1_design = DotDict(custom1_design)
        custom1_design.fu = lambda N_data: (
            np.random.normal(0, 1.5, N_data).reshape(-1, 1)
        )
        custom1_design.fz = lambda N_data: (
            np.random.normal(0, 1.5, N_data).reshape(-1, 1)
        )
        custom1_design.fx = lambda z, u, N_data: (
            u
            + z
            + custom1_design.noise_level * np.random.normal(0, 1, N_data).reshape(-1, 1)
        )
        custom1_design.fy = lambda x, u, N_data: (
            scipy.expit(x)
            + 0.05 * u
            + custom1_design.noise_level * np.random.normal(0, 1, N_data).reshape(-1, 1)
        )
        custom1_design.fm = lambda x, N_data: (
            x
            + custom1_design.noise_level_merror
            * np.random.normal(0, 1, N_data).reshape(-1, 1)
        )
        custom1_design.fn = lambda x, N_data: (
            x
            + custom1_design.noise_level_merror
            * np.random.normal(0, 1, N_data).reshape(-1, 1)
            + custom1_design.bias
        )
        custom1_design.ydox = lambda x, u, N_data: scipy.expit(x)
    ...
    def construct_NP(self):  # Newey-Powell 'sigmoid' design
        ...
        NP_design.futw = lambda N_data: (
            np.random.multivariate_normal(NP_design.MU, NP_design.SIGMA, size=N_data)
        )
        ...
        NP_design.fx = lambda z, u, N_data: (
            stats.norm.cdf(
                (NP_design.utw[:, 1] + NP_design.utw[:, 2]) / np.sqrt(2)
            ).reshape(-1, 1)
        )
        NP_design.fy = lambda x, u, N_data: (
            np.log(np.abs(16 * x - 8) + 1) * np.sign(x - 0.5) + u
        )
        NP_design.fm = lambda x, N_data: (
            x
            + NP_design.noise_level_merror
            * np.random.normal(0, 1, N_data).reshape(-1, 1)
        )
        NP_design.fn = lambda x, N_data: (
            x
            + NP_design.noise_level_merror
            * np.random.normal(0, 1, N_data).reshape(-1, 1)
            + NP_design.bias
    ...
    def construct_HLLT(self):  # demand design
        ...
        HLLT_.ft = lambda N_data: (
            np.random.uniform(0, 1, HLLT_.N_data).reshape(-1, 1) * 10
        )

        HLLT_.fu = lambda N_data: (
            np.random.normal(0, 1, HLLT_.N_data).reshape(-1, 1)
        )  # noise price
        ...
        HLLT_.fm = lambda x, N_data: np.reshape(
            x[:, HLLT_.merror_dim]
            + HLLT_.noise_level_merror * np.random.normal(0, 1, N_data),
            newshape=(-1, 1),
        )
        HLLT_.fn = lambda x, N_data: np.reshape(
            x[:, HLLT_.merror_dim]
            + HLLT_.noise_level_merror * np.random.normal(0, 1, N_data)
            + HLLT_.bias,
            newshape=(-1, 1),
        )
        HLLT_.fy = lambda x, u, N_data: np.reshape(
            100
            + (10 + x[:, 0]) * x[:, 2] * get_psi(x[:, 1])
            - 2 * x[:, 0]
            + np.random.normal(HLLT_.rho * u, np.sqrt(1 - HLLT_.rho**2), HLLT_.N_data),
            newshape=(-1, 1),
        )
        HLLT_.ydox = lambda x: (
            100 + (10 + x[:, 0]) * x[:, 2] * get_psi(x[:, 1]) - 2 * x[:, 0]
        )
