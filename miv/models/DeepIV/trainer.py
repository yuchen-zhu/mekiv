...
class DeepIVTrainer(object):
    ...
    def __init__(
        ...
        dropout_rate = min(1000.0 / (1000.0 + data_configs["data_size"]), 0.5)
        ...
        self.instrumental_opt = torch.optim.Adam(
            self.instrumental_net.parameters(), weight_decay=0.0001
        )
        self.response_opt = torch.optim.Adam(
            self.response_net.parameters(), weight_decay=0.001
        )
    ...
    def train(self, rand_seed: int = 42, verbose: int = 0) -> float:
        ...
        mdl = DeepIVModel(
            self.response_net, self.instrumental_net, self.data_config["data_name"]
        )
        if self.gpu_flg:
    ...
    @staticmethod
    def density_est_loss(
        cat: OneHotCategorical, norm: Normal, treatment: torch.Tensor
    ) -> torch.Tensor:
    ...
    def update_stage1(self, train_data: TrainDataSetTorch, verbose: int):
        ...
        for t in range(self.n_epochs):
            ...
            for instrumental, treatment in data_loader:
                ...
                treatment = preprocess.rescale_treatment(
                    treatment, self.data_config["data_name"]
                )
    ...
    def update_stage2(self, train_data: TrainDataSetTorch, verbose: int):
        ...
        if train_data.covariate is not None:
            data_set = TensorDataset(
                train_data.instrumental, train_data.covariate, train_data.outcome
            )
        loss_val = None
        self.instrumental_net.train(False)
        for t in range(self.n_epochs):
            ...
            for data in data_loader:
                ...
                if train_data.covariate is not None:
                    covariate = data[1]
                outcome = preprocess.rescale_outcome(
                    outcome, self.data_config["data_name"]
                )
                outcome = outcome.repeat((self.n_sample, 1))
                norm, cat = self.instrumental_net(instrumental)
                if torch.sum(torch.isnan(cat.probs)):
                    raise RuntimeError("NaN prob detected")
                pred = DeepIVModel.sample_from_density(
                    self.n_sample, self.response_net, norm, cat, covariate
                )
