...
def compute_cme(all_args, output, incl_obs):
    K = get_K(
        all_args, output=output, incl_Xobs=incl_obs
    )  # because all staeg 3 data inherits from stage 1
    if output == "X":
        lambd_star = (
            all_args.stage_3.lambd_x
            if not incl_obs
            else np.exp(cme_tuning(K, all_args.stage_3))
        )
    elif (
        (output == "MN")
        or (output == "N")
...
def compute_f(all_args, cme_X):
    ...
    brac2_inv = np.linalg.inv(
        make_psd(W_star @ W_star.T)
        + all_args.stage_1.n_dev * xi_star * make_psd(K_X.XX)
    )
...
def compute_f_main(all_args, which_cme_Xall):
    if which_cme_Xall == "cme_Xall":
        ...
        f = compute_f(
            all_args, all_args.stage_3[which_cme_Xall]
        )  # if which_cme_Xall = cme_Mall or cme_Nall or cme_avMNall, that means we are using these as stand-in's for X.
        return f
...
def create_stage_2_raw_data(all_args):
    ...
    Chi = sample_from_khat(
        all_args.stage_2.n_Chi, all_args.stage_1.cme_N.K.sig_X
    ).reshape(-1, 1)

    ### gamma ###
    K_Zz = compute_rbf_kernel(
        all_args.stage_1.train.Z, all_args.stage_1.dev.Z, all_args.stage_1.cme_N.K.sig_Z
    )
    # gamma_N = all_args.stage_1.cme_N.brac_inv.dot(K_Zz)
    n = all_args.stage_1.cme_N.K.ZZ.shape[0]
    gamma_N = np.linalg.solve(
        all_args.stage_1.cme_N.K.ZZ + n * all_args.stage_1.cme_N.lambd_star * np.eye(n),
        K_Zz,
    )
    # gamma_MN = all_args.stage_1.cme_MN.brac_inv.dot(K_Zz)
    gamma_MN = np.linalg.solve(
        all_args.stage_1.cme_MN.K.ZZ
        + n * all_args.stage_1.cme_MN.lambd_star * np.eye(n),
        K_Zz,
    )
    #############
    ...
    cos_term = np.cos(
        Chi @ all_args.stage_1.train.N.reshape(1, -1)
    ...
    numer.cos_weighted = cos_term.dot(
        gamma_MN * all_args.stage_1.train.M.reshape(-1, 1)
    )  # shape: Chi.shape[0] x args.dev.Z.shape[0]
    numer.sin_weighted = sin_term.dot(
        gamma_MN * all_args.stage_1.train.M.reshape(-1, 1)
    )
    ...
    z_dev_flat = np.repeat(
        Z2[np.newaxis, :, :], all_args.stage_2.n_Chi, axis=0
    ).reshape(-1, z_dim)
...
def prepare_stage_2_data_and_args(all_args, raw_data2):
    ...
    throw_away_outliers_with_labelrealparts(raw_data2, all_args.stage_2.label_cutoff)
    shuffle_(raw_data2)
    ...
    for key in all_args.stage_1.train.keys():
        key1 = key + "1"
        all_args.stage_2[key1] = tensor(
            all_args.stage_1.train[key]
        )  # tensorise for stage 2 training
...
def prepare_stage_3_data_and_args(all_args, stage_two_out):
    ...
    if not all_args.stage_2.learn_lambd_x:
        ...
        all_args.stage_3.lambd_x = (
            all_args.stage_3.lambd_n
        )  # use lambd_n as heuristic if not learning lambd_x
    else:
...
class StageTwo(torch.nn.Module):
    def __init__(self, args):
        super().__init__()
        logger.info("first 10 M: ", args.M1[:10])
        logger.info("first 10 N: ", args.N1[:10])
        logger.info("first 10 _X: ", args._X1[:10])
        ...
        self.cme_X.brac_inv = torch.inverse(
            torch.tensor(make_psd(args.cme_N.K.ZZ))
            + args.M1.shape[0] * self.cme_X.lambd_star * torch.eye(args.M1.shape[0])
        )
    ...
    def forward(self, idx):
        ...
        numer.cos_weighted = torch.sum(
            cos_term * gamma.T * self.x.reshape(1, -1), dim=-1
        ).reshape(-1, 1)
        numer.sin_weighted = torch.sum(
            sin_term * gamma.T * self.x.reshape(1, -1), dim=-1
        ).reshape(-1, 1)
        numer.value = numer.cos_weighted + numer.sin_weighted * 1j
        #################
...
def split_into_batches(args):
    ...
    print("num train data (chi): ", len(idxes))
    np.random.shuffle(idxes)

    batch_i = 0
    while True:
        batches_idxes.append(
            torch.tensor(
                idxes[batch_i * args.batch_size : (batch_i + 1) * args.batch_size]
            )
        )
        batch_i += 1
...
def train(model, args):
    ...
    for ep in range(args.num_epochs):
        ...
        for i, batch_idx in enumerate(batches_idx):
            ...
            if i % 1 == 0:
                print(
                    "[epoch %d, batch %5d] loss: %.5f, mse: %.5f, reg: %.5f"
                    % (
                        ep + 1,
                        i + 1,
                        running_loss / 1,
                        mse / 1,
                        args.reg_param * reg / 1,
                    )
                )
                # breakpoint()
                running_loss = 0.0
