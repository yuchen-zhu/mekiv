...
class StageMModel(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        logger.info("first 10 M: ", stage1_MNZ.M[:10])
        logger.info("first 10 N: ", stage1_MNZ.N[:10])
        ...
        if not train_params["lambda_x"]:
            self.params = torch.nn.Parameter(
                torch.cat(
                    [self.x_initialiser.flatten(), torch.tensor([np.log(0.05)])], dim=0
                )
            )  # stage1_size x 1
    ...
    def forward(self, idx):
        ...
        if not self.train_params["lambda_x"]:
            ...
            gamma_x = torch.linalg.solve(
                self.KZ1Z1 + n * self.lambda_x * torch.eye(n), K_Z1z
            )
        ...
        numer.sin_weighted = torch.sum(
            sin_term * gamma_x.t() * self.x.reshape(1, -1), dim=-1
        ).reshape(-1, 1)
    ...
    while True:
        batches_idxes.append(
            torch.tensor(
                idxes[
                    batch_i
                    * stageM_args.batch_size : (batch_i + 1)
                    * stageM_args.batch_size
                ]
            )
        )
        batch_i += 1
...
def stage_m_train(
    ...
    for ep in range(stageM_args.num_epochs):
        ...
        batches_idx = split_into_batches(
            stageM_data=stageM_data, stageM_args=stageM_args
        )
