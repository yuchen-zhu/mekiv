...
class LVM(torch.nn.Module):
    ...
    def stage_2_loss(self, idx):
        ...
        preds = torch.mean(self.response(x_samples_from_z.reshape(-1, dim_x)).reshape(self.sample_size_from_pxz, -1),
                           axis=0).reshape(-1, 1)
        #         breakpoint()
...
def build_net_for_linear_and_sigmoid(data, sample_size_from_pxz):
