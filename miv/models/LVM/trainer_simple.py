...
class Encoder(nn.Module):  # q(x_hidden|m,n,z)
    def __init__(self, dropout_ratio):
        ...
        # self.feature_net = Feature(dropout_ratio, n_input_dim=2)
    ...
    def forward(self, instrument, m_obs, n_obs):
        ...
        # feature = self.feature_net(torch.cat([instrument, m_obs], dim=1))
...
class MDecoder(nn.Module):  # p(m|x_hidden)
    ...
    def forward(self, treatment):
        ...
        logsigma = self.logsigma_linear(feature)
        # print(torch.min(torch.exp(logsigma)))
        norm = Normal(loc=mu, scale=torch.exp(logsigma)+1e-10)
        return norm
...
class LVMTrainer:
    ...
    def __init__(self, data_configs: Dict[str, Any], train_params: Dict[str, Any],
        ...
        self.response_opt = torch.optim.Adam(self.response_net.parameters(), lr=1e-8,
                                             weight_decay=0.001)
    ...
    def update_stage1(self, train_data: TrainDataSetTorch, verbose: int):
        ...
        for t in range(self.n_epochs):
            ...
            for data in data_loader:
                ...
                recon_m_prob = torch.sum(log_pmx)
                recon_n_prob = torch.sum(log_pnx)

                # kld = E[log(p(x|z)) - log(q(x|z,m,n))]_q
                # breakpoint()
                pxz = self.iv_net_hidden(iv)
                # print("2")
                log_pxz = torch.sum(torch.clamp(pxz.log_prob(x_samples_from_q), min=-1*clamp_val, max=clamp_val))
                # print("3")
                log_qxzmn = torch.sum(torch.clamp(qxzmn.log_prob(x_samples_from_q), min=-1*clamp_val, max=clamp_val))
                # print("4")
                ...
                true_log_pmx = torch.log(torch.sum(torch.exp(true_pmx_dist.log_prob(m_obs_reps)).reshape(sample_size, -1), dim=0))
                true_log_pnx = torch.log(torch.sum(torch.exp(true_pnx_dist.log_prob(n_obs_reps)).reshape(sample_size, -1), dim=0))

                true_loglikelihood = -1 * (torch.sum(true_log_pmx) + torch.sum(true_log_pnx))
                # breakpoint()
