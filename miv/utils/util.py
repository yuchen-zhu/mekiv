...
def cme_tuning(
    K, args
):  # returns the best hyperparameter lambd_star for CME regression
    if isinstance(args.log_lambd_0, list):
        n = K.XX.shape[0]
        lambd_list = np.exp(np.linspace(args.log_lambd_0[0], args.log_lambd_0[1], 50))
        gamma_list = [
            np.linalg.solve(K.ZZ + n * lambd * np.eye(n), K.Zz) for lambd in lambd_list
        ]
        score = [
            np.trace(gamma.T.dot(K.XX.dot(gamma)) - 2 * K.xX.dot(gamma))
        ...
        def callbackF(log_lambd, Nfeval=Nfeval):
            print(
                "{0}   {1}   {2}".format(
                    Nfeval[0], log_lambd, cme_loss(np.exp(log_lambd), K, args)
                )
            )
            Nfeval[0] = Nfeval[0] + 1
...
def cme_loss(
    lambd, K, args
):  # objective for tuning lambda in KRR for CME, see KIV paper A.5.2
    n = args.n_train
    m = args.n_dev
...
def f_tuning(cme_X, all_args):
    ...
    if isinstance(all_args.stage_3.log_xi_0, list):
        ...
        def callbackF(log_xi, Nfeval=Nfeval):
            print(
                "{0}   {1}   {2}".format(
                    Nfeval[0], log_xi, f_loss(np.exp(log_xi), cme_X, all_args)
                )
            )
            Nfeval[0] = Nfeval[0] + 1
...
def get_K(all_args, output, incl_Xobs):
    ...
    if (output == "N") or (output == "M") or (output == "_X"):  # done in stage 1
        ...
        X = np.concatenate(
            [all_args.stage_1.train.M, all_args.stage_1.train.N], axis=-1
        )
    ...
    if incl_Xobs:  # stage 3
        X = swap_merror_dim(
            Xall=all_args.train._Xall, x_=X, merror_dim=all_args.merror_dim
        )
        x = swap_merror_dim(
            Xall=all_args.test._Xall, x_=x, merror_dim=all_args.merror_dim
        )
    ...
    K["xx"] = compute_rbf_kernel(x, x, sig_X)
    if incl_Xobs:
        K["Xtest"] = compute_rbf_kernel(
            X, X_test, sig_X
        )  # only done in test time, which would include all observed dims of X as well.
...
def visualise_dataset(dataset, sample_size, observed_keys):  # dataset is a dict object
    ...
    for key in dataset.keys():
        if dataset[key] is None:
            continue
        if len(dataset[key].shape) == 1 or (
            len(dataset[key].shape) == 2 and dataset[key].shape[-1] == 1
        ):
            ...
            (
                O_cols.append(vis_key)
                if observed
                else print("{} is latent.".format(vis_key))
            )
            ...
            vis_data = [
                dataset[key][:sample_size, i] for i in range(dataset[key].shape[-1])
            ]
...
def visualise_cme(size, target_vars, cme, args, design):
    ...
    ehat_K_xX = K_xX.dot(
        cme.brac_inv.dot(K_Zz)
    )  # \hat{E}_X[k(x, X)]. shape: [size_x_vals, size_z]
...
def visualise_char_fun(size, target_vars, cme, args, design):
    curly_x = np.linspace(-0.3, 0.3, size).reshape(-1, 1)
    #     z_vals = args.train.Z[np.random.choice(np.arange(args.n_train), size=size, replace=False)]
    z_vals = np.linspace(
        args.train.Z[:1000].min(), args.train.Z[:1000].max(), size
    ).reshape(-1, 1)
...
def curate_data(splitted_data_and_args, raw_data):
    ...
    for key in raw_data.keys():
        ...
        splitted_data_and_args.train[_key] = raw_data[key][
            : splitted_data_and_args.n_train
        ]
        splitted_data_and_args.dev[_key] = raw_data[key][
            splitted_data_and_args.n_train : splitted_data_and_args.n_train
            + splitted_data_and_args.n_dev
        ]
        splitted_data_and_args.test[_key] = raw_data[key][
            splitted_data_and_args.n_train
            + splitted_data_and_args.n_dev : splitted_data_and_args.n_train
            + splitted_data_and_args.n_dev
